...
DECLARE @satistarih As DATE = '20180409'
DECLARE @stokkod AS varchar(30) ='PN404749'
DECLARE @stokmiktar as DECIMAL(12,3) = 24
--DECLARE @
...
end) as stok_miktari from [STOK_HAREKETLERI_GIRIS_CIKIS] WHERE sth_stok_kod=@stokkod and sth_tarih<=@satistarih --and sth_depono=@depokodu
...
case when sth_giris_cikis=0 THEN sth_miktar
when sth_giris_cikis=1 THEN sth_miktar*-1 
end) as stok_miktari from [STOK_HAREKETLERI_GIRIS_CIKIS] WHERE sth_stok_kod=@stokkod and sth_tarih=@satistarih and sth_evraktip=4 --and sth_depono=@depokodu
group by sth_stok_kod ),0.0))
...
PRINT 'Envanter Miktar:' + CAST (@envanterMiktar as varchar(50))
-- Eğer envanter miktarı negatif ise, envanter miktarını tarihteki satis miktarı kadar alır 
-- bu satır gereksiz olmuş, neg. olması aşağıdaki döngü pas geçmesini sağlar
if(@envanterMiktar<0) Set @envanterMiktar=@tarihsatismiktar
PRINT 'Tarih Satis Miktar:' + CAST (@tarihsatismiktar as varchar(50))
...
WHERE sth_evraktip=3 and sth_stok_kod=@stokkod
